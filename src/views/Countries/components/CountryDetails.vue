<template>
  <div class="flex-column">
    <div
      class="cursor-pointer text-3xl px-2 py-1 rounded text-blue-400 active:scale-90 my-3 max-w-[5em] m-auto mt-10"
    ...
    <div v-if="!isLoading">
      <div
        class="bg-blue-400 rounded text-white w-[20em] md:w-[40em] py-3 px-1 mt-4"
      >
        <div class="text-8xl">{{ countryDetails?.emoji }}</div>
        <div class="flex items-center m-3">
          <div class="w-1/4 inline-block text-left font-bold">Name:</div>
        ...
        <div class="flex items-center m-3">
          <div class="w-1/4 inline-block text-left font-bold">Code:</div>
          <div class="w-3/4 md:text-left">
            {{ countryDetails?.code ?? "N/A" }}
          </div>
        </div>
        <div class="flex items-center m-3">
          <div class="w-1/4 inline-block text-left font-bold">Continent:</div>
          <div class="w-3/4 md:text-left">
            {{ countryDetails?.continent?.name ?? "N/A" }}
          </div>
        </div>
        <div class="flex items-center m-3">
          <div class="w-1/4 inline-block text-left font-bold">Capital:</div>
          <div class="w-3/4 md:text-left">
            {{ countryDetails?.capital ?? "N/A" }}
          </div>
        </div>
        <div class="flex items-center m-3">
          <div class="w-1/4 inline-block text-left font-bold">Languages:</div>
          <div class="w-3/4 md:text-left">
            {{
        ...
        <div class="flex items-center m-3">
          <div class="w-1/4 inline-block text-left font-bold">Currencies:</div>
          <div class="w-3/4 md:text-left">
            {{ countryDetails?.currencies.join(", ") ?? "N/A" }}
          </div>
        </div>
        <div class="flex items-center m-3">
          <div class="w-1/4 md:w-1/4 inline-block text-left font-bold">
            Phone Code:
          </div>
          <div class="w-3/4 md:text-left">
